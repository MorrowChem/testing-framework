...
if "NSLOTS" in os.environ:
    nmpi=os.environ["NSLOTS"]
else:
    nmpi=8

# keywords related to this model (except k-points, which are handled separately)
vasp_keywords = { 'encut' : 200.0, 'xc' : 'PBE', 'ismear' : 0, 'sigma' : 0.1, 
                 'nelm' : 150, 'ialgo' : 53, 'time':0.05, 'amix' : 0.03, 'bmix' : 0.01, 'ivdw': 12 }
# keywords related to accuracy
vasp_keywords.update( { 'ediff' : 1.0e-4, 'prec' : 'Accurate', 'addgrid' : False, 'lreal' : True } )
...
vasp_keywords.update( { 'lscalapack' : True, 'lplane' : True, 'kpar' : 1, 
    'ncore' : 1 } )
