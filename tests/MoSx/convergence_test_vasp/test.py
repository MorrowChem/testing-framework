...
for i, kpts in enumerate(kpts_range):
    model.calculator.set(kspacing=kpts, directory='kpts/{:04d}'.format(i))
    bulk_copy = bulk.copy()
...
for i, cutoff in enumerate(cutoff_range):
    model.calculator.set(encut=cutoff, kspacing=default_kspacing, directory='cutoffs/{:04d}'.format(i))
    bulk_copy = bulk.copy()
    bulk_copy.set_calculator(model.calculator)
...
properties = {'kpts': kpts_range.tolist(), 'cutoff': cutoff_range.tolist(), 'es_kpts': es_kpts, 'es_cutoff': es_cutoff}
